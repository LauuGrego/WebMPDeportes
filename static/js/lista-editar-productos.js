...
async function fetchProducts(page = 1, append = false) {
  try {
    ...
    const response = await fetch(`http://127.0.0.1:8000/productos/listar?page=${page}&limit=${productsPerPage}`, {
      headers: {
...
async function searchProducts(searchTerm, append = false) {
  try {
    ...
    const response = await fetch(`http://127.0.0.1:8000/productos/listar?search=${encodeURIComponent(searchTerm)}&page=${currentPage}&limit=${productsPerPage}`, {
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('authToken')}`
      }
    });
...
async function saveProductChanges(event) {
  ...
  try {
    const resp = await fetch(
      `http://127.0.0.1:8000/productos/actualizar/${productId}`,
      {
        method:  'PUT',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('authToken')}`
...
async function deleteProduct(productId) {
  ...
  try {
    const response = await fetch(`http://127.0.0.1:8000/productos/eliminar/${productId}`, {
      method: 'DELETE',
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('authToken')}`
      }
